...
export const AddContext = createContext<IAddContext>({coin: null, setCoin: () => null})

const AddProvider: FunctionComponent<PropsWithChildren<unknown>> = ({ children}) => {
    const [coin, setCoin] = useState<ICoin | ICoinInfo | null>(null)
    return <AddContext.Provider value={ {coin, setCoin}}>
        {children}
    </AddContext.Provider>
}
