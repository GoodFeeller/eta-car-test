...
const BuyCoin: FunctionComponent<{coin: ICoin | ICoinInfo}> = ({ coin }) => {
    ...
    useEffect( () => setError(count.split('').some( e => isNaN(Number.parseInt(e)))), [count])
    return <div className={styles.buyBlock}>
        <input className={error ? styles.error : styles.correct} type={'text'} maxLength={4} onChange={ e => setCount(e.target.value)} value={count}/>
        <button onClick={() => buy()} disabled={error || count.length === 0}>Buy</button>
    </div>
}
